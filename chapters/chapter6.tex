...
\par
\textbf{Antes ou depois de efetuar o login}
\par
O participante ainda antes de efetuar o login pode alterar algumas opções de configuração, entre elas temos: o dispositivo ao qual se irá conectar (Figura \ref{f:devicesettings}), os tipos de dados que são para recolher e a sua relação temporal em relação à atividade física(Figura \ref{f:recordsettings}).
\par




\textbf{Antes de efetuar o login}
\par
Para isso então temos como atividade Principal uma lista de sessões que à primeira vista aparece vazia. Ela aparece vazia (Figura \ref{f:noLogin}), pois o Login tem que ser Efetuado. 



Para ser efetuado o Login pode ser de duas maneiras diferentes (Figura \ref{f:canLogin}): Iniciar uma nova sessão de medições(é verificado que não tem login efetuado); escolhendo a opção para efetuar Login.



Caso não tenha uma conta para acesso esta pode ser criada utilizando menu lateral (Figura \ref{canLogin}). 



Para efetuar a criação da conta tem que indicar um utilizador e password, o utilizador tem que estar disponível (Figura \ref{f:createaccount}). 


Pode então proceder ao Login através da atividade desenvolvida para o Login\ref{f:dologin}.
\par
\textbf{Depois de efetuar o login}
\par
Depois de o login estar estabelecido, o participante pode então criar novas sessões de medições (Figura \ref{f:onNewSession}), pode parar quando quiser e irá voltar a visualizar a sua lista de medições com o acréscimo da última sessão efetuada. 

A qualquer momento o participante pode verificar as medições efetuadas relativas a cada sessão \ref{f:onreadSession}. Pode ser feito o logout no menu lateral esquerdo e login novamente com o mesmo participante ou com outro diferente.
