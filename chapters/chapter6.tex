...
Todos os esquemas de dados disponíveis estão no diretório \cite{schemas-available} e estes são os tipos de dados que são utilizados para validar os  \textit{dataPoints} inseridos, ou seja, estes são todos aqueles que podem ser inseridos na coleção do  \textit{MongoDB}.
...
\subsection{Como instalar este \textit{backend}}
...
Com este Menu o participante pode efetuar o registo e efetuar o \textit{Login} na aplicação (Figura \ref{f:leftpannel}).
\begin{figure}[H]
\centering
...
O participante antes de efetuar \textit{Login} tem a atividade Principal praticamente vazia. Pode aceder ao menu lateral (Figura \ref{f:leftpannel}), ou às configurações (Figura \ref{f:seesettings}).
\begin{figure}[H]
\centering
...
\subsubsection{Atividade para efetuar o \textit{Login}}
