...
O \gls{VJ} é um dispositivo médico que conjuga a tecnologia têxtil com soluções avançadas de engenharia biomédica, que permite uma monitorização contínua durante 72 horas seguidas. O dispositivo permite a configuração para adquirir diferentes sinais vitais tais como \gls{ECG}, frequência cardíaca e ainda acelerómetro. É uma t-shirt confortável e fácil de vestir que vem equipada com um dispositivo que recolhe dados e os permite partilhar via Bluetooth em tempo real para os dispositivos móveis. \cite{vj}
...
Foi ainda feito um breve estudo em relação a outros três possíveis backends, não foi tão aprofundado como os apresentados anteriormente mas ainda assim serve para análise comparativa na seguinte tabela.
\begin{table}[H]
\centering
...
Depois de analisados seis serviços de backend para dar suporte a uma aplicação móvel na àrea de saúde, faz-se aqui uma breve analise comparativa das suas características. Cada um apresenta vantagens nuns aspetos e desvantagens noutros.
\par 
...
% nesta secção, apresentar o PRODUTO proposto: um sistema para apoiar projetos de I&D que incluem recolha de dados fisiológicos, que podem ocorrer em laboratório, como em ambulatório Identificar os atores envolvidos e as suas motivações Oct 12, 2017 12:01 PM

\section{Casos de Uso e Requisitos}
Para a utilização da framework selecionada foi desenvolvidas duas aplicações. Uma aplicação móvel que tem como objetivo principal recolher dados dos sensores, e uma aplicação Web para visualizar esses dados recolhidos. O dispositivo utilizado para recolher os dados foi o Vital Jacket e os dados possíveis a ser recolhidos são: frequência cardíaca, \gls{ECG} e acelerómetro. Foi utilizado este dispositivo pois como já fez parte de outros estudos, é um equipamento válido e permitia recolher os tipos de dados que pretendíamos.

As aplicações possuem dois tipos de utilizadores distintos:
\begin{itemize}
  \item Revisor/Investigador que tem como objetivo rever dados inseridos pelos diferentes utilizadores
  \item Utilizador que tem como função recolher dados vitais e acelerómetro para posteriormente serem revistos pelo revisor/investigador
\end{itemize}

\subsection{Aplicação Móvel}
O objetivo principal da aplicação móvel é através do Vital Jacket recolher várias sessões de leitura para serem posteriormente analisados e visualizados por investigadores/Revisores.

\subsubsection{Casos de Uso}
\textcolor{red}{{\huge TODO}}
%dividir o capitulo 3 em 3 subsecoes: - Visão geral do sistema - Casos de utilização na colheita de dados - Casos de utilização na revisão de casos Oct 12, 2017 12:04 PM
Na figura seguinte podemos visualizar todos os casos de uso (cenários-objetivo) da aplicação móvel. O principal ator desta app é o utilizador(alvo de estudo).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{imgs/app-and-usecase.PNG}
  \caption[Diagrama de casos de uso da aplicação móvel]{Diagrama de casos de uso da aplicação móvel}
  
  \label{f:usecaseandroidapp}
\end{figure}

\subsection{Aplicação Web}
O objetivo principal da aplicação web é visualizar os dados inseridos pelos utilizadores.

\subsubsection{Casos de Uso}

Na figura seguinte podemos visualizar todos os casos de uso (cenários-objetivo) da aplicação web. O principal ator desta app é o revisor/investigador.
...
\section{Frameworks seleccionadas}
A escolha de um backend para a solução era um ponto importante. Das várias frameworks disponíveis para um possível backend tanto para a aplicação móvel como para a aplicação web foram selecionadas três para ser desenvolvido um protótipo com o objetivo de concluir se seria uma boa hipótese para a solução, entre elas estão: \gls{OMH}, \gls{FHIR} e GoogleFit.
...
\section{Resultados e lições aprendias}
\textcolor{red}{{\huge TODO}}//todo
...
\chapter{ Mover para a secção correta}
